...
namespace FogGerenciadorDeVendas.Dominio.Nfce
{
    public class GeradorDeNfce
    {
        ...
        public NFe.Classes.NFe GerarNfce(int numeroDaNota, List<Produto> produtos)
        {
            ...
            return ObterNfeValidade(numeroDaNota, produtos);
        }

        private NFe.Classes.NFe ObterNfeValidade(int numero, List<Produto> produtos)
        {
        ...
        private infNFe ObterInf(int numero, List<Produto> produtos)
        {
            ...
            for (var i = 0; i < produtos.Count; i++)
            {
                ...
                infNFe.det.Add(GetDetalhe(i, infNFe.emit.CRT, modeloDoDocumento, produto));
            }
        ...
        protected virtual det GetDetalhe(int i, CRT crt, ModeloDocumento modelo, Produto produto)
        {
            var det = new det
            {
                nItem = i + 1,
                prod = GetProduto(produto),
                imposto = new imposto
                {
                    vTotTrib = 0.17m,
